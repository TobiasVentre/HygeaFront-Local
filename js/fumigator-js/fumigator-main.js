// fumigator-main.js
// Módulo principal para inicializar el panel del fumigator

import { 
    fumigatorState,
    loadFumigatorContext, 
    loadFumigatorData,
    getId,
    getValue,
    formatTime
...
import { 
    updateFumigatorHeader 
} from './fumigator-ui.js';

import { 
    initializeSidebarNavigation, 
    initializeQuickActions 
} from './fumigator-navigation.js';

import { 
    initializeConsultationDateFilter,
    createConsultationItemElement,
    initializeFumigatorRescheduleModal
} from './fumigator-appointments.js';

import { 
    initializeProfileEditing 
} from './fumigator-profile.js';

import { 
    initializePrescriptionModal 
} from './fumigator-prescriptions.js';

/**
 * Inicializa el panel del fumigator
 */
export async function initializeFumigatorPanel() {
    console.log('🚀 Inicializando panel del fumigator...');
    
    try {
        // 1. Cargar contexto del usuario
        await loadFumigatorContext();
        
        // 2. Esperar un momento si los datos no están listos
        if (!fumigatorState.currentUser?.firstName || !fumigatorState.currentUser?.lastName) {
            await new Promise(resolve => setTimeout(resolve, 500));
            const { state } = await import('../state.js');
            fumigatorState.currentUser = state.user;
        }
        
        // 3. Cargar datos del fumigator
        const fumigatorData = await loadFumigatorData();
        
        // 4. Actualizar header
        updateFumigatorHeader(fumigatorData);
        ...
        console.log('✅ Panel del fumigator inicializado correctamente');
        
    } catch (error) {
        console.error('❌ Error en la inicialización del panel del fumigator:', error);
        updateFumigatorHeader(null);
    }
}

/**
 * Carga las estadísticas del fumigator
 */
export async function loadFumigatorStats() {
    try {
        const fumigatorId = fumigatorState.currentFumigatorData?.fumigatorId;
        if (!fumigatorId) {
            console.warn('No hay fumigatorId disponible para cargar estadísticas');
            return;
        }
        ...
        const clientsToday = document.getElementById('clients-today');
        ...
        if (clientsToday) {
            clientsToday.textContent = todayAppointments?.length || 0;
        }
        ...
        try {
            ...
            const prescriptionsResponse = await ApiClinical.get(`v1/Prescription/fumigator/${fumigatorId}`).catch(() => []);
...
async function loadTodayConsultationsForDashboard() {
    ...
    try {
        ...
        for (const apt of allAppointments) {
            ...
            try {
                ...
                apt.clientName = `${client.Name || client.name || ''} ${client.lastName || client.LastName || ''}`.trim() || 'Cliente sin nombre';
            } catch (err) {
                console.warn('Error al cargar cliente:', err);
                apt.clientName = 'Cliente desconocido';
            }
        }
    ...
    setTimeout(async () => {
        const { initializeAttendButtons } = await import('./fumigator-appointments.js');
...
async function loadWeeklySchedule() {
    ...
    try {
        const fumigatorId = getId(fumigatorState.currentFumigatorData, 'fumigatorId');
        if (!fumigatorId) {
            console.warn('No hay fumigatorId disponible para cargar agenda');
