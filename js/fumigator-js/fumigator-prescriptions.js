 ...
 */
export function openPrescriptionModal(clientName = null, clientId = null, encounterId = null, appointmentId = null) {
    console.log('💊 openPrescriptionModal llamado', { clientName, clientId, encounterId, appointmentId });
    ...
    if (!modal) {
        ...
        const checkModal = () => {
            ...
            if (modal) {
                ...
                showPrescriptionModal(modal, clientName, clientId, encounterId, appointmentId);
 ...
 */
function showPrescriptionModal(modal, clientName, clientId, encounterId, appointmentId) {
    if (!modal) {
    ...
    // Prellenar el nombre del cliente si se proporciona
    if (clientName) {
        const clientNameInput = document.getElementById('prescription-client-name');
        if (clientNameInput) {
            clientNameInput.value = clientName;
            console.log('Nombre del cliente prellenado:', clientName);
        }
    }
    
    // Guardar IDs en campos ocultos
    if (clientId) {
        updateClientIdField(clientId);
        console.log('ClientId guardado:', clientId);
    } else {
        updateClientIdField(null);
    }
...
async function handlePrescriptionSubmit(e) {
    ...
    if (!prescription.client || !prescription.diagnosis || !prescription.medication || 
    ...
    const currentFumigatorData = state.fumigatorData;
    
    if (!currentFumigatorData?.fumigatorId) {
    ...
    try {
        ...
        const prescriptionData = {
            ...
            fumigatorId: currentFumigatorData.fumigatorId,
        ...
        showNotification(`Receta generada exitosamente para ${prescription.client}`, 'success');
...
/**
 * Inicializa el autocompletado de clientes
 */
function initializeClientAutocomplete() {
    const clientInput = document.getElementById('prescription-client-name');
    const suggestionsContainer = document.getElementById('client-suggestions');
    ...
    clientInput.addEventListener('input', async function(e) {
        ...
        searchTimeout = setTimeout(async () => {
            try {
                ...
                displayClientSuggestions(clients, suggestionsContainer, clientInput);
            } catch (error) {
    ...
    clientInput.addEventListener('keydown', function(e) {
        const suggestions = suggestionsContainer.querySelectorAll('.client-suggestion-item');
        const activeSuggestion = suggestionsContainer.querySelector('.client-suggestion-item.active');
...
async function searchClients(searchTerm) {
    try {
        ...
        const clients = await Api.get('v1/Client/all');
        ...
        const filtered = clients.filter(client => {
            const firstName = (client.name || client.Name || '').toLowerCase();
            const lastName = (client.lastName || client.LastName || '').toLowerCase();
            const dni = (client.dni || client.Dni || '').toString();
        ...
        console.error('Error al buscar clientes:', error);
        showNotification('Error al buscar clientes. Intenta nuevamente.', 'error');
        return [];
    }
}

/**
 * Muestra las sugerencias de clientes
 */
function displayClientSuggestions(clients, container, input) {
    container.innerHTML = '';
    
    if (clients.length === 0) {
        container.innerHTML = '<div style="padding: 1rem; text-align: center; color: #6b7280;">No se encontraron clientes</div>';
    ...
    clients.forEach(client => {
        ...
        const dni = client.dni || client.Dni || 'N/A';
        const fullName = `${firstName} ${lastName}`.trim() || 'Cliente sin nombre';
        
        const suggestionItem = document.createElement('div');
        suggestionItem.className = 'client-suggestion-item';
...
/**
 * Actualiza el campo oculto con el ID del cliente
 */
function updateClientIdField(clientId) {
    const form = document.getElementById('prescription-form');
    if (!form) return;
    
    let hiddenInput = document.getElementById('prescription-client-id');
    if (!hiddenInput) {
        hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden';
        hiddenInput.id = 'prescription-client-id';
        hiddenInput.name = 'client-id';
...
export async function loadPrescriptionsView() {
    ...
    // Verificar que tenemos el fumigatorId
    let fumigatorId = getId(currentFumigatorData, 'fumigatorId');
    if (!fumigatorId) {
        console.warn('No hay fumigatorId disponible para cargar recetas');
        ...
        <div class="fumigator-id-error">
            <i class="fas fa-exclamation-circle fumigator-id-error-icon"></i>
            <p>No se pudo obtener el ID del fumigator. Por favor, recarga la página.</p>
        </div>
    `;
    ...
    // Cargar recetas
    await loadFumigatorPrescriptions(fumigatorId);
}

/**
 * Carga las prescripciones del fumigator
 */
async function loadFumigatorPrescriptions(fumigatorId) {
    ...
    try {
        ...
        // Obtener recetas del fumigator
        const prescriptions = await ApiClinical.get(`v1/Prescription/fumigator/${fumigatorId}`);
        ...
        // Obtener información de clientes para cada receta
        const prescriptionsWithClients = await Promise.all(
            prescriptions.map(async (prescription) => {
                let clientName = 'Cliente desconocido';
                let clientDni = '';
                
                try {
                    const clientId = prescription.clientId || prescription.ClientId;
                    if (clientId) {
                        const client = await Api.get(`v1/Client/${clientId}`);
                        const name = client.name || client.Name || '';
                        const lastName = client.lastName || client.LastName || '';
                        clientName = `${name} ${lastName}`.trim() || 'Cliente sin nombre';
                        clientDni = client.dni || client.Dni || '';
                    }
                } catch (err) {
                    console.warn('Error al cargar información del cliente:', err);
                }
        ...
        // Renderizar recetas
        renderPrescriptionsList(prescriptionsWithClients);
    } catch (error) {
...
function renderPrescriptionsList(prescriptions) {
    ...
    prescriptionsList.innerHTML = prescriptions.map(p => {
        ...
        const clientName = p.clientName || 'Cliente desconocido';
        const clientDni = p.clientDni || '';
    ...
    <div class="prescription-item">
        ...
        <div class="prescription-header">
            ...
            <div class="prescription-info">
                <h4 class="prescription-client">${clientName}</h4>
                ...
                <div class="prescription-meta">
                    ...
                    <span class="prescription-dni">
                        <i class="fas fa-id-card"></i> DNI: ${clientDni}
                    </span>` : ''}
                </div>
            </div>
        </div>
