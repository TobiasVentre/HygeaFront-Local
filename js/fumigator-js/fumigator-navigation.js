...
export async function handleSectionNavigation(section) {
    ...
    const profileSection = document.getElementById('fumigatorProfileSection');
    ...
    try {
        ...
        try {
            const { initializeAttendButtons, initializeFumigatorRescheduleModal } = 
                await import('./fumigator-appointments.js');
...
async function showProfileSection(mainDashboard, profileSection) {
    ...
    if (profileSection) {
        try {
            ...
            if (!currentFumigatorData) {
                const { loadFumigatorData } = await import('./fumigator-core.js');
                await loadFumigatorData();
            }
            
            const { updateFumigatorProfileSection, setProfileFormEditable } = await import('./fumigator-ui.js');
            updateFumigatorProfileSection(currentFumigatorData);
...
async function loadConsultasSection() {
    ...
    try {
        const { loadTodayConsultationsView } = await import('./fumigator-appointments.js');
...
async function loadRecetasSection() {
    ...
    try {
        const { loadPrescriptionsView } = await import('./fumigator-prescriptions.js');
...
async function loadAgendaSection() {
    ...
    try {
        const { loadAgendaView } = await import('./fumigator-schedule.js');
...
async function loadClientesSection() {
    console.log('👥 Cargando sección de clientes');
    
    try {
        const { loadClientsView } = await import('./fumigator-appointments.js');
        await loadClientsView();
    } catch (error) {
        console.error('❌ Error al cargar clientes:', error);
        showNotificationError('Error al cargar los clientes');
    }
}
...
export function initializeQuickActions() {
    ...
    if (viewClientsBtn) {
        const newBtn = viewClientsBtn.cloneNode(true);
        viewClientsBtn.parentNode.replaceChild(newBtn, viewClientsBtn);
        
        newBtn.addEventListener('click', function() {
            console.log('👥 Navegando a clientes');
            setActiveNav('clientes');
            handleSectionNavigation('clientes');
        });
    }
    ...
    if (manageScheduleBtn) {
        ...
        newBtn.addEventListener('click', async function() {
            ...
            try {
                const { openScheduleManager } = await import('./fumigator-schedule.js');
...
function showNotificationError(message) {
    try {
        const { showNotification } = require('./fumigator-ui.js');
