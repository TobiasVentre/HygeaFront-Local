...
export async function renderAgendaContent(agendaSection) {
    ...
    try {
        ...
        const appointments = await ApiScheduling.get(
            `Appointments?doctorId=${doctorId}&startTime=${today.toISOString()}&endTime=${threeMonths.toISOString()}`
        );
...
async function loadDoctorAvailability(modal, doctorId) {
    try {
        const { ApiScheduling } = await import('../api.js');
        const availability = await ApiScheduling.get(`DoctorAvailability/search?doctorId=${doctorId}`);
...
async function openAvailabilityForm(parentModal, doctorId, availabilityId = null) {
    ...
    if (availabilityId) {
        const { ApiScheduling } = await import('../api.js');
        availability = await ApiScheduling.get(`DoctorAvailability/${availabilityId}`);
...
async function saveAvailability(formModal, parentModal, doctorId, availabilityId) {
    ...
    try {
        ...
        let response;
        if (availabilityId) {
            response = await ApiScheduling.patch(`DoctorAvailability/${availabilityId}`, data);
            showNotification('Horario actualizado', 'success');
        } else {
            response = await ApiScheduling.post(`DoctorAvailability/${doctorId}`, data);
...
async function deleteAvailability(modal, doctorId, availabilityId) {
    ...
    try {
        const { ApiScheduling } = await import('../api.js');
        await ApiScheduling.delete(`DoctorAvailability/${availabilityId}`);
