...
export async function openClientVideoCall(appointmentId, fumigatorId, fumigatorName) {
    try {
        console.log('📹 Abriendo videollamada para cliente:', { appointmentId, fumigatorId, fumigatorName });
        ...
        // Crear modal
        const modal = createVideoCallModal(appointmentId, fumigatorName);
        modal.setAttribute('data-appointment-id', appointmentId);
        document.body.appendChild(modal);
        
        // Inicializar videollamada
        await initializeClientVideoCall(modal, appointmentId, fumigatorId);
...
function createVideoCallModal(appointmentId, fumigatorName) {
    ...
    modal.innerHTML = `
        ...
        ">
            ...
            ">
                <div>
                    <h3 style="margin: 0; font-size: 1.25rem; font-weight: 600;">
                        <i class="fas fa-video"></i> Videollamada con ${fumigatorName}
                    </h3>
            ...
            <div id="client-video-call-container" style="
                flex: 1;
                background: #000;
                position: relative;
                display: flex;
                ...
                min-height: 400px;
            ">
                <p id="client-video-loading" style="text-align: center; font-size: 1rem;">
                    <i class="fas fa-spinner fa-spin"></i> Conectando a la videollamada...
                </p>
            </div>
            
            <div id="client-video-controls" style="
                padding: 1rem 1.5rem;
                background: #f9fafb;
                ...
                flex-wrap: wrap;
            ">
                <button id="client-toggle-mic" style="
                    background: #3b82f6;
                    color: white;
                    border: none;
                ...
                <button id="client-toggle-camera" style="
                    background: #3b82f6;
                    color: white;
                    border: none;
                    padding: 0.75rem 1.5rem;
                ...
                <button id="client-end-call" style="
                    background: #ef4444;
                    color: white;
                    border: none;
                    padding: 0.75rem 1.5rem;
    ...
    const endCallBtn = modal.querySelector('#client-end-call');
...
async function initializeClientVideoCall(modal, appointmentId, fumigatorId) {
    try {
        const videoContainer = modal.querySelector('#client-video-call-container');
        const videoLoading = modal.querySelector('#client-video-loading');
        ...
        // Obtener token del cliente desde el backend
        const { ApiScheduling } = await import('../api.js');
        const clientId = appState.currentClient?.clientId || appState.currentClient?.ClientId;
        ...
        const tokenResponse = await ApiScheduling.get(`Video/token/${appointmentId}?userId=client-${clientId}&isOwner=false`);
        ...
        console.log('✅ Token obtenido para cliente');
        ...
        startClientVideoCall(videoContainer, roomUrl, token, modal, appointmentId);
        
    } catch (error) {
        console.error('❌ Error al inicializar videollamada del cliente:', error);
...
async function startClientVideoCall(videoContainer, roomUrl, token, modal, appointmentId) {
    try {
        ...
        setupClientVideoControls(modal, callFrame, appointmentId);
        
        // Trazas útiles (registrar ANTES del join)
        callFrame.on('joining-meeting', () => {
            console.log('🔵 joining-meeting');
            const loading = videoContainer.querySelector('#client-video-loading');
        ...
        // CRÍTICO: Detectar cuando el fumigator se une a la videollamada
        callFrame.on('participant-updated', (e) => {
            console.log('[participant-updated]', e);
            
            // Verificar si hay participantes (el fumigator)
            try {
                const participants = callFrame.participants();
                console.log('📊 Participantes en la videollamada:', participants);
                
                // Buscar si hay algún participante que no sea el cliente actual
                const clientId = appState.currentClient?.clientId || appState.currentClient?.ClientId;
        ...
        callFrame.join({ url: validUrl, token })
            .then(() => {
                console.log('✅ Cliente unido a la videollamada');
                const loading = videoContainer.querySelector('#client-video-loading');
                if (loading) loading.style.display = 'none';
...
function setupClientVideoControls(modal, callFrame, appointmentId) {
    const toggleMic = modal.querySelector('#client-toggle-mic');
    const toggleCamera = modal.querySelector('#client-toggle-camera');
    const endCall = modal.querySelector('#client-end-call');
...
/**
 * Verifica si el fumigator está en la videollamada usando la API del backend (que usa Daily.co directamente)
 */
async function checkFumigatorInVideoCall(appointmentId, fumigatorId) {
    try {
        const { ApiScheduling } = await import('../api.js');
        const clientId = appState.currentClient?.clientId || appState.currentClient?.ClientId;
        ...
        if (existingModal) {
            return false; // Ya hay un modal de videollamada abierto
                        ...
                        console.log(`📹 Verificando si el fumigator está en la videollamada para appointment ${appointmentId} usando API de Daily.co...`);
                        ...
                        try {
                            const endpoint = `Video/check-fumigator/${appointmentId}`;
                            ...
                            const hasFumigator = checkResponse.hasFumigator || checkResponse.HasFumigator || false;
                            
                            console.log(`📊 Resultado de verificación (API Daily.co):`, {
                                appointmentId: appointmentId,
                                hasFumigator: hasFumigator,
        ...
        console.error(`❌ Error completo al verificar si el fumigator está en la videollamada (appointment ${appointmentId}):`, {
            error: error,
...
export function startVideoCallMonitoring() {
    ...
    videoCallMonitorInterval = setInterval(async () => {
        try {
            ...
            if (!clientId) {
                console.log('⚠️ No hay clientId disponible para monitoreo');
                return;
            }

            // Traigo solo turnos in_progress
            const appointmentsResponse = await ApiScheduling.get(`Appointments?clientId=${clientId}&status=in_progress`);
            ...
            for (const appointment of sortedAppointments) {
                ...
                if (!appointmentId || !fumigatorId) {
                    console.log('⚠️ Appointment sin appointmentId o fumigatorId:', appointment);
                    continue;
                }
