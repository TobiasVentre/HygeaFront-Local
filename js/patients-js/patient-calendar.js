...
export async function loadAvailableDatesAndTimes(doctorId) {
    try {
        ...
        const appointmentsResponse = await ApiScheduling.get(
            `Appointments?doctorId=${doctorId}&startTime=${now.toISOString()}&endTime=${fourWeeksLater.toISOString()}`
        );
...
export async function loadAvailableTimes(doctorId, selectedDate) {
    try {
        ...
        const appointmentsResponse = await ApiScheduling.get(
            `Appointments?doctorId=${doctorId}&startTime=${startOfDay.toISOString()}&endTime=${endOfDay.toISOString()}`
        );
        const appointments = Array.isArray(appointmentsResponse) 
            ? appointmentsResponse 
            : (appointmentsResponse?.value || appointmentsResponse || []);
