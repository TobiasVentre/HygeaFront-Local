...
import { buildProfileData, toISODate } from './client-utils.js';
import { updateWelcomeBanner } from './client-dashboard.js';

/**
 * Carga y muestra el perfil del cliente
 */
export async function loadClientProfile() {
    ...
    if (!appState.currentClient) {
        const { loadClientData } = await import('./client-data.js');
        await loadClientData();
    }

    const profileData = buildProfileData(appState.currentClient, appState.currentUser);
...
function createProfileSection(profileData) {
    ...
    section.setAttribute('data-client', JSON.stringify(data));
    
    setTimeout(() => {
        const editBtn = section.querySelector('#editProfileBtn');
        if (editBtn) {
            editBtn.addEventListener('click', function() {
                const savedData = JSON.parse(section.getAttribute('data-client') || '{}');
 ...
 */
export function createProfileViewHTML(client) {
    return `
                ...
                <div class="profile-section">
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-field">
                            ...
                            <div class="field-value">${client.birthDate || '-'}</div>
                        </div>
                ...
                <div class="profile-section">
                    ...
                    <div class="info-grid">
                        <div class="info-field">
                            ...
                            <div class="field-value">${client.medicalInsurance || 'No especificada'}</div>
                        </div>
                        <div class="info-field">
                            <label class="field-label">Número de Afiliado</label>
                            <div class="field-value">${client.insuranceNumber || 'No especificado'}</div>
                        </div>
                    </div>
                </div>
            </div>
 ...
 */
export function createProfileEditHTML(client) {
    return `
                    ...
                    <div class="profile-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="edit-lastName" name="LastName" value="${client.lastName || ''}" required>
                            </div>

                            <div class="form-field">
                                <label for="edit-email">Email</label>
                                <input type="email" id="edit-email" name="Email" value="${client.email || ''}" disabled>
                                <small class="field-hint">El email no puede ser modificado</small>
                            </div>

                            <div class="form-field">
                                <label for="edit-phone">Teléfono</label>
                                <input type="tel" id="edit-phone" name="Phone" value="${client.phone || ''}" placeholder="+54 9 11 1234-5678">
                            </div>

                            <div class="form-field">
                                <label for="edit-dni">DNI</label>
                                <input type="text" id="edit-dni" name="Dni" value="${client.dni || ''}" required>
                            </div>

                            <div class="form-field">
                                <label for="edit-birthDate">Fecha de Nacimiento</label>
                                <input type="date" id="edit-birthDate" name="DateOfBirth" value="${toISODate(client.birthDate) || ''}" required>
                            </div>

                            <div class="form-field full-width">
                                <label for="edit-address">Dirección</label>
                                <input type="text" id="edit-address" name="Adress" value="${client.address || ''}" placeholder="Calle, número, ciudad">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="profile-section">
                        ...
                        <div class="form-grid">
                            <div class="form-field">
                                ...
                                <input type="text" id="edit-HealthPlan" name="HealthPlan" value="${client.medicalInsurance || ''}" placeholder="Ej: OSDE, Swiss Medical">
                            </div>

                            <div class="form-field">
                                <label for="edit-MembershipNumber">Número de Afiliado</label>
                                <input type="text" id="edit-MembershipNumber" name="MembershipNumber" value="${client.insuranceNumber || ''}" placeholder="123456789">
                            </div>
                        </div>
                    </div>
 ...
 */
export function toggleProfileEdit(clientData) {
    ...
    if (isEditing) {
        ...
        profileContent.innerHTML = createProfileViewHTML(clientData);
        editBtn.innerHTML = '<i class="fas fa-edit"></i> Editar Perfil';
        editBtn.className = 'btn btn-secondary';
        if (profileSection) {
            profileSection.setAttribute('data-client', JSON.stringify(clientData));
        }
    } 
    else {
        // Cambiar a modo edición
        profileContent.innerHTML = createProfileEditHTML(clientData);
        ...
        if (form) {
            form.addEventListener('submit', async function (e) {
                e.preventDefault();
                await saveProfileChanges(form, clientData);
            });
        }

        if (cancelBtn) {
            cancelBtn.addEventListener('click', function () {
                toggleProfileEdit(clientData);
            });
        }
    }
}
