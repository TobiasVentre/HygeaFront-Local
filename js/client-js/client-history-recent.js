// ============================================
// CLIENTE HISTORY RECENT - VERSION DEBUG
// ============================================

import { appState, getAuthenticatedUser } from './client-state.js';
...
export async function loadRecentClientHistory() {
    ...
    try {
        ...
        console.log('🔍 Cargando historial reciente para clientId:', clientId);
        console.log('📅 Rango de fechas:', from.toISOString(), 'hasta', now.toISOString());

        const response = await ApiClinical.get(
            `v1/Encounter?clientId=${clientId}&from=${from.toISOString()}&to=${now.toISOString()}`
        );
        ...
        // Obtener info de fumigatores
        const fumigatorIds = [...new Set(lastThree.map(e => e.FumigatorId || e.fumigatorId))];
        const fumigatorsMap = new Map();

        console.log('👨‍⚕️ Cargando información de', fumigatorIds.length, 'fumigatores');
        ...
        for (const id of fumigatorIds) {
            try {
                ...
                console.warn(`⚠️ Error al cargar fumigator ${id}:`, err);
                fumigatorsMap.set(id, `Dr. ${id}`);
            }
        }
        ...
        historyList.innerHTML = lastThree
            .map(enc => {
                ...
                const clientId = enc.ClientId || enc.clientId;
                const date = new Date(enc.Date || enc.date);
                const fumigatorName = fumigatorsMap.get(enc.FumigatorId || enc.fumigatorId) || 'Dr. Desconocido';
                ...
                return `
                    <div class="history-compact-card">
                        <div class="history-compact-content">
                            <div class="history-compact-left">
                                <div class="history-compact-date">
                                    ...
                                    ${dateStr}
                                    <span class="history-fumigator-separator">|</span>
                        ...
                        <button class="btn-history-view" onclick="viewPrescription(${encounterId || 'null'}, ${appointmentId || 'null'}, ${clientId || 'null'})">
