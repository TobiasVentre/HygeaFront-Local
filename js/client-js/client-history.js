...
export async function loadClientHistoryFull() {
    ...
    try {
        const clientId = appState.currentClient?.clientId || appState.currentUser?.userId;
        ...
        const encountersResp = await ApiClinical.get(`v1/Encounter?clientId=${clientId}&from=${from.toISOString()}&to=${now.toISOString()}`);
        ...
        // Obtener información de fumigatores en paralelo
        const fumigatorIds = [...new Set(encounters.map(e => e.fumigatorId || e.FumigatorId).filter(Boolean))];
        ...
        // Renderizar
        renderHistoryCards(encounters, container, fumigatorsMap);
...
/** Renderizar cards */
function renderHistoryCards(encounters, container, fumigatorsMap) {
    ...
    encounters.forEach(enc => {
        ...
        const fumigatorInfo = fumigatorsMap.get(fumigatorId) || { 
        ...
        clone.querySelector('.history-fumigator .meta-value').textContent = fumigatorInfo.fullName;
        clone.querySelector('.history-fumigator .meta-specialty').textContent = 
            fumigatorInfo.specialty ? `— ${fumigatorInfo.specialty}` : '';
        ...
        doc.text("Informe de Consulta", margin, y);
        ...
        doc.text(fumigatorInfo.fullName, margin + 35, y);
        y += lineHeight;

        if (fumigatorInfo.specialty) {
            doc.setFont("helvetica", "bold");
            doc.text("Especialidad:", margin, y);
            doc.setFont("helvetica", "normal");
            doc.text(fumigatorInfo.specialty, margin + 35, y);
            y += lineHeight;
        }
