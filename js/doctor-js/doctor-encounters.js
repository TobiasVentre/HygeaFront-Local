...
async function saveEncounter(modal, appointmentId, patientId) {
    try {
        ...
        try {
            ...
            // Obtener userId del doctor y paciente
            const { Api } = await import('../api.js');
            const { doctorState } = await import('./doctor-core.js');
            const currentUserId = doctorState.currentUser?.userId || doctorState.currentUser?.UserId || doctorState.currentUser?.id || doctorState.currentUser?.Id;
            ...
            if (appointment.doctorId || appointment.DoctorId) {
                ...
                try {
                    ...
                    console.warn('⚠️ No se pudo obtener doctorUserId, usando currentUserId:', err);
            ...
            // Obtener patientUserId
            let patientUserId = null;
            if (appointment.patientId || appointment.PatientId) {
                const patId = appointment.patientId || appointment.PatientId;
                try {
                    const patient = await Api.get(`v1/Patient/${patId}`);
                    patientUserId = patient?.userId || patient?.UserId;
                } catch (err) {
                    console.warn('⚠️ No se pudo obtener patientUserId:', err);
                }
            }
        ...
        try {
            ...
            const { generateHl7SummaryIfNeeded } = await import('./doctor-appointments.js');
...
async function initializeVideoCall(modal, appointmentId, patientId, patientName) {
    try {
        ...
        try {
            ...
            console.log('📹 Solicitando sala de videollamada...', { appointmentId, doctorId, patientId });
            ...
            // Obtener token por separado para el doctor (con isOwner=true)
            // El doctor debe ser owner para poder iniciar la reunión
            console.log('📹 Obteniendo token para el doctor...', { appointmentId, doctorId });
            let token = null;
            try {
                const tokenResponse = await ApiScheduling.get(`Video/token/${appointmentId}?userId=doctor-${doctorId}&isOwner=true`);
                    token = tokenResponse.token || tokenResponse.Token;
                    console.log('✅ Token obtenido para el doctor (con owner):', token ? `Presente (${token.length} caracteres)` : 'Faltante');
            } catch (tokenError) {
                console.error('❌ Error al obtener token para el doctor:', tokenError);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const doctorId = getId(doctorState.currentDoctorData, 'doctorId');
                    const userName = `doctor-${doctorId}`;
